...
#include_next <debug/formatter.h>
#else
// SAM中也有此文件，但声明了旧版的_Safe_iterator，所以只能弃用
...
namespace __gnu_debug
{
	...
	template <typename _Iterator>
	_GLIBCXX_CONSTEXPR bool __check_singular(_Iterator const &);
	...
	public:
		...
		struct _Parameter
		{
			...
			{
				_M_variant._M_iterator._M_type = _GLIBCXX_TYPEID(__it);
				_M_variant._M_iterator._M_state = _S_reverse_state(_M_variant._M_iterator._M_state);
			}
			...
			_Parameter(std::reverse_iterator<_Safe_iterator<_Iterator, _Sequence,
				...
				: _Parameter(__it.base(), __name, _Is_iterator{})
			{
				_M_variant._M_iterator._M_type = _GLIBCXX_TYPEID(std::reverse_iterator<_Iterator>);
				_M_variant._M_iterator._M_state = _S_reverse_state(_M_variant._M_iterator._M_state);
			}
			...
			_Parameter(std::move_iterator<_Safe_iterator<_Iterator, _Sequence,
				...
				: _Parameter(__it.base(), __name, _Is_iterator{})
			{
				_M_variant._M_iterator._M_type = _GLIBCXX_TYPEID(std::move_iterator<_Iterator>);
			}
	...
	private:
		_Error_formatter(const char *__file, unsigned int __line,
			...
			: _M_file(__file), _M_line(__line), _M_num_parameters(0), _M_text(0), _M_function(__function)
#if _GLIBCXX_HAVE_STACKTRACE
#ifdef _GLIBCXX_DEBUG_BACKTRACE
			  ,
			  _M_backtrace_state(__glibcxx_backtrace_create_state(0, 1, _S_err, 0)), _M_backtrace_full(&__glibcxx_backtrace_full)
#else
			  ,
