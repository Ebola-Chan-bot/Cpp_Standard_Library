...
#include_next <debug/safe_iterator.h>
#else
// SAM也有此文件，但_Safe_iterator少一个模板参数，只能弃用原文件
...
#define _GLIBCXX_DEBUG_VERIFY_OPERANDS(_Lhs, _Rhs, _BadMsgId, _DiffMsgId)                                                               \
  ...
  {                                                                                                                                     \
    _GLIBCXX_DEBUG_VERIFY((!_Lhs._M_singular() && !_Rhs._M_singular()) || (_Lhs._M_value_initialized() && _Rhs._M_value_initialized()), \
...
namespace __gnu_debug
{
  ...
  public:
          ...
          _Safe_base()
    {
      ...
      _GLIBCXX_DEBUG_VERIFY(!__x._M_singular() || __x._M_value_initialized(),
    ...
    _GLIBCXX_NODISCARD
        _GLIBCXX20_CONSTEXPR friend bool
  ...
  public:
    ...
    operator[](difference_type __n) const _GLIBCXX_NOEXCEPT
    {
      if (!std::__is_constant_evaluated())
      {
        _GLIBCXX_DEBUG_VERIFY(this->_M_can_advance(__n) && this->_M_can_advance(__n + 1),
...
#if __cpp_lib_three_way_comparison
    [[nodiscard]]
    _GLIBCXX20_CONSTEXPR friend auto
