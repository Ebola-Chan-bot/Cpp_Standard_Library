#pragma once
// SAM也有此文件，但_Safe_iterator少一个模板参数（类外成员函数定义不能使用模板参数的默认值），只能弃用原文件
...
#include_next <debug/safe_iterator.tcc>
#else
namespace __gnu_debug
{
		...
		_M_get_distance_from_begin() const
	{
		...
		typename _Distance_traits<_Iterator>::__type __res = __get_distance(_M_get_sequence()->_M_base().begin(), base());
		...
		_M_get_distance_to_end() const
	{
		...
		typename _Distance_traits<_Iterator>::__type __res = __get_distance(base(), _M_get_sequence()->_M_base().end());
					   ...
					   bool __check_dereferenceable) const
	{
		...
		if (__dist.second != __dp_equality)
		{
			...
			return __dist.first == 0 || (__dist.first > 0 && (!__check_dereferenceable || _M_dereferenceable()));
		}
								 ...
								 _Distance_precision> &__dist) const
	{
		...
		if (this->_M_singular() || __rhs._M_singular() || !this->_M_can_compare(__rhs))
			return false;
		...
		// If range is not empty first iterator must be dereferenceable.
		return __dist.first == 0 || (__dist.first > 0 && this->_M_dereferenceable());
	}
} // namespace __gnu_debug
...
namespace std _GLIBCXX_VISIBILITY(default)
{
	...
	__niter_base(const ::__gnu_debug::_Safe_iterator<_Ite, _Seq,
													 std::random_access_iterator_tag> &__it) noexcept(std::is_nothrow_copy_constructible<_Ite>::value)
	{
					  ...
					  const ::__gnu_debug::_Safe_iterator<_Ite, _Seq, _Cat> &__result)
	{
		...
		if (__dist.second > ::__gnu_debug::__dp_sign && __result._M_can_advance(__dist.first, true))
				...
				const ::__gnu_debug::_Safe_iterator<_II2, _Seq2, _Cat2> &__first2)
	{
		...
		if (__dist.second > ::__gnu_debug::__dp_sign && __first2._M_can_advance(__dist.first, true))
