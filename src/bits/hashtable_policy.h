...
namespace std _GLIBCXX_VISIBILITY(default)
{
  ...
  namespace __detail
  {
    ...
    struct _Node_iterator_base
    {
      ...
      friend bool
      operator==(const _Node_iterator_base &__x, const _Node_iterator_base &__y) noexcept
      {
    ...
    struct _RehashStateGuard
    {
      ...
      _RehashStateGuard(_RehashPolicy &__policy)
          : _M_guarded_obj(std::__addressof(__policy)), _M_prev_state(__policy._M_state())
      {
      }
                        ...
                        const _NodeGetter &__node_gen, false_type __uks)
    {
      ...
      __pair_type __do_rehash = __h._M_rehash_policy._M_need_rehash(__h._M_bucket_count,
    ...
    protected:
      ...
      _M_bucket_index(const _Hash_node_value<_Value, false> &__n,
          ...
          noexcept(noexcept(declval<const _Hash &>()(declval<const _Key &>())) && noexcept(declval<const _RangeHash &>()((__hash_code)0,
                                                                                                                         (std::size_t)0)))
      {
    ...
    protected:
      ...
      _Local_iterator_base(const _Local_iterator_base &__iter)
          : __node_iter_base(__iter._M_cur), _M_bucket(__iter._M_bucket), _M_bucket_count(__iter._M_bucket_count)
      {
        if (_M_bucket_count != size_t(-1))
          _M_init(*__iter._M_h());
      }
    ...
    protected:
      ...
      {
        return _S_node_equals(__lhn, __rhn) && _M_key_equals(_ExtractKey{}(__lhn._M_v()), __rhn);
      }
        ...
        _M_equal(const __hashtable &__other) const
    {
      ...
      for (auto __x_n = __this->_M_begin(); __x_n;)
      {
        ...
        for (; __x_n_end && __this->key_eq()(_ExtractKey{}(__x_n->_M_v()),
