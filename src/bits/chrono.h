...
namespace std _GLIBCXX_VISIBILITY(default)
{
	...
	namespace chrono
	{
		...
		template <typename _Tp>
		using __enable_if_is_duration = typename enable_if<__is_duration<_Tp>::value, _Tp>::type;

		template <typename _Tp>
		using __disable_if_is_duration = typename enable_if<!__is_duration<_Tp>::value, _Tp>::type;
		...
		_GLIBCXX_NODISCARD constexpr __enable_if_is_duration<_ToDur>
		...
		inline constexpr bool
			is_clock_v<_Tp, void_t<typename _Tp::rep, typename _Tp::period,
								   ...
								   decltype(_Tp::now())>> = __and_v<is_same<typename _Tp::duration,
																			duration<typename _Tp::rep, typename _Tp::period>>,
		 ...
		 */
		template <typename _Rep, typename _Period>
		[[nodiscard]] constexpr enable_if_t<numeric_limits<_Rep>::is_signed, duration<_Rep, _Period>>
		...
		class duration
		{
			...
			using __is_harmonic = __bool_constant<__divide<_Period2, _Period>::den == 1>;
		...
		using __common_rep_t = typename enable_if<is_convertible<const _Rep2 &, _CRep>::value, _CRep>::type;
...
#undef _GLIBCXX_CHRONO_INT64_T
		...
		_GLIBCXX_NODISCARD constexpr __enable_if_t<__is_duration<_ToDur>::value, time_point<_Clock, _ToDur>>
		 ...
		 */
		template <typename _ToDur, typename _Clock, typename _Dur>
		[[nodiscard]] constexpr enable_if_t<__is_duration_v<_ToDur>, time_point<_Clock, _ToDur>>
		 ...
		 */
		template <typename _ToDur, typename _Clock, typename _Dur>
		[[nodiscard]] constexpr enable_if_t<__is_duration_v<_ToDur> && !treat_as_floating_point_v<typename _ToDur::rep>,
		...
		template <>
		inline constexpr bool is_clock_v<steady_clock> = true;
		template <>
		inline constexpr bool is_clock_v<file_clock> = true;
		/// @}
#endif // C++20
	} // namespace chrono
...
#pragma GCC diagnostic pop
	...
	namespace filesystem
	{
		...
		protected:
			// For internal use only
			template <typename _Dur>
			static chrono::time_point<__file_clock, common_type_t<_Dur, chrono::seconds>>
