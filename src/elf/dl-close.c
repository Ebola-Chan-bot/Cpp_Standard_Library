...
void _dl_close_worker(struct link_map *map, bool force)
{
	...
	/* If _dl_close is called recursively (some destructor call dlclose),
	   ...
	   again and return.  */
	static enum { not_pending,
				  pending,
				  rerun } dl_close_state;

	if (map->l_direct_opencount > 0 || map->l_type != lt_loaded || dl_close_state != not_pending)
	{
...
retry:
	...
	while (++done_index < nloaded)
	{
		...
		if (l->l_type == lt_loaded && l->l_direct_opencount == 0 && !l->l_nodelete_active
			/* See CONCURRENCY NOTES in cxa_thread_atexit_impl.c to know why
			   acquire is sufficient and correct.  */
			&& atomic_load_acquire(&l->l_tls_dtor_count) == 0 && !l->l_map_used)
			continue;
	...
	for (unsigned int i = 0; i < nloaded; ++i)
	{
		...
		else if (imap->l_type == lt_loaded)
		{
			...
			for (size_t cnt = 0; imap->l_scope[cnt] != NULL; ++cnt)
				...
				{
					struct link_map *tmap = (struct link_map *)((char *)imap->l_scope[cnt] - offsetof(struct link_map, l_searchlist));
...
#define SCOPE_ELEMS(imap) \
	(sizeof(imap->l_scope_mem) / sizeof(imap->l_scope_mem[0]))
				...
				if (imap->l_scope != imap->l_scope_mem && remain < SCOPE_ELEMS(imap))
				{
			...
			if (imap->l_loader != NULL && imap->l_loader->l_idx != IDX_STILL_USED)
				imap->l_loader = NULL;
	...
	if (!RTLD_SINGLE_THREAD_P && (unload_global || scope_mem_left || (GL(dl_scope_free_list) != NULL && GL(dl_scope_free_list)->count)))
	{
		THREAD_GSCOPE_WAIT();
	...
	for (unsigned int i = first_loaded; i < nloaded; ++i)
	{
		...
		if (!imap->l_map_used)
		{
			...
			if (__glibc_unlikely(imap->l_tls_blocksize > 0))
			{
				...
				if (GL(dl_tls_dtv_slotinfo_list) != NULL && !remove_slotinfo(imap->l_tls_modid,
				...
				if (imap->l_tls_offset != NO_TLS_OFFSET && imap->l_tls_offset != FORCED_DYNAMIC_TLS_OFFSET)
				{
					/* Collect a contiguous chunk built from the objects in
					   ...
					   reclaim it.  */
#if TLS_TCB_AT_TP
					if (tls_free_start == NO_TLS_OFFSET || (size_t)imap->l_tls_offset == tls_free_start)
					{
						/* Extend the contiguous chunk being reclaimed.  */
						tls_free_start = imap->l_tls_offset - imap->l_tls_blocksize;

						if (tls_free_end == NO_TLS_OFFSET)
							tls_free_end = imap->l_tls_offset;
					}
					else if (imap->l_tls_offset - imap->l_tls_blocksize == tls_free_end)
					...
					else
					{
					   ...
					   one block right away.  */
						if (tls_free_end == GL(dl_tls_static_used))
						{
							...
							tls_free_end = imap->l_tls_offset;
							tls_free_start = tls_free_end - imap->l_tls_blocksize;
						}
						else if ((size_t)imap->l_tls_offset == GL(dl_tls_static_used))
							GL(dl_tls_static_used) = imap->l_tls_offset - imap->l_tls_blocksize;
					...
					if (tls_free_start == NO_TLS_OFFSET)
					{
						...
						/* Extend the contiguous chunk being reclaimed.  */
						tls_free_end = imap->l_tls_offset + imap->l_tls_blocksize;
					else if (imap->l_tls_offset + imap->l_tls_blocksize == tls_free_start)
						/* Extend the chunk backwards.  */
						tls_free_start = imap->l_tls_firstbyte_offset;
					/* This isn't contiguous with the last chunk freed.
					   One of them will be leaked unless we can free
					   one block right away.  */
					else if (imap->l_tls_offset + imap->l_tls_blocksize == GL(dl_tls_static_used))
			...
			if (force)
			{
				...
				if (entries != NULL)
				{
					...
					for (idx = 0; idx < size; ++idx)
					{
					   ...
					   object.  */
						if (entries[idx].name != NULL && entries[idx].map == imap)
						{
